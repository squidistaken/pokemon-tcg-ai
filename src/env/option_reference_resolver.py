...
class OptionReferenceResolver:
    ...
    def resolve(
        ...
        if option_type in (
            ...
            if option_type == OptionType.TOOL_CARD:
                attached = OptionReferenceResolver._card_in_list(
                    pokemon.tools, option.toolIndex
                )
            else:
                attached = OptionReferenceResolver._card_in_list(
                    pokemon.energyCards, option.energyIndex
                )
    ...
    def _card_id_at(
        ...
        :return: Card ID, or 0 when the reference is absent, face-down, out of
            range, or in a zone the agent cannot see.
        """
    ...
    def _pokemon_at(
        ...
        :param index: Index within the area, or None.
        :return: The Pokemon, or None when the reference is absent, holds a
            face-down card, or falls outside the referenced board area.
        """
        if area is None or index is None:
            return None
        player = state.players[player_index]
    ...
    def _card_in_list(cards: list, index: int | None) -> int:
        ...
        :return: The card's ID, or 0 when the reference is absent, face-down,
            or falls outside the list.
        """
        if index is None:
            return 0
