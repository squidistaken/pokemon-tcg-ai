...
class CardDatabase:
    ...
    def __init__(self) -> None:
        ...
        self._card_features = torch.zeros(n_card_rows, self.CARD_FEATURES, dtype=torch.float32)
        self._card_cats = torch.zeros(n_card_rows, self.CARD_CATEGORICALS, dtype=torch.int64)
        ...
        self._attack_features = torch.zeros(n_attack_rows, self.ATTACK_FEATURES, dtype=torch.float32)
        for attack in attacks:
            row = attack.attackId
            cost_counts = [0.0] * self.ENERGY_TYPES
    ...
    def card_features(self) -> torch.Tensor:
        ...
        :return: Float32 tensor of shape ``(max_card_id + 1, CARD_FEATURES)``.
        """
    ...
    def card_cats(self) -> torch.Tensor:
        ...
        :return: Int64 tensor of shape ``(max_card_id + 1, CARD_CATEGORICALS)``.
        """
    ...
    def attack_features(self) -> torch.Tensor:
        ...
        :return: Float32 tensor of shape ``(max_attack_id + 1, ATTACK_FEATURES)``.
        """
