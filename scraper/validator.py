"""Replicate the native engine's deck-legality rules in Python.

Mirrors ``ApiBattleStart`` in ``ptcg_engine/ptcgProgram 22/Api.h`` (constants
``DECK_SIZE=60``, ``DECK_SAME_CARD_MAX=4``). ``cg/game.py`` only checks the deck
length, so the remaining rules must be enforced here before saving a deck.
"""
