#!/bin/bash
# Launch the local 10M-frame deck-pinned finetune under the crash supervisor.
#
# Overlays conf/experiment/deck_pinned_150m_local.yaml on
# conf/ppo_fixed_deck_finetune.yaml. Every hyperparameter lives in those two
# files; this script owns only what the supervisor needs as flags: which
...
#   ./scripts/train_deck_pinned_150m_local.sh env.num_workers=8 agent.lr=1e-4
#   RESUME=1 ./scripts/train_deck_pinned_150m_local.sh
#
...
CONFIG_NAME="ppo_fixed_deck_finetune"
EXPERIMENT="deck_pinned_150m_local"

GROUP="${GROUP:-deck-pinned-150m-local}"
RUN_NAME="${RUN_NAME:-tf-ptr-pinned-150m-10m-s42}"
# Absolute target, not an increment: the supervisor subtracts the warm start's
# 150,011,904 frames and collects exactly 10M more, ending at 160,011,904.
TOTAL_FRAMES="${TOTAL_FRAMES:-160011904}"
ATTEMPTS="${ATTEMPTS:-10}"

RUN_DIR="outputs/$GROUP/$RUN_NAME"

# Weights-only warm start from the 150M snapshot. The supervisor reads its
# frame count so TOTAL_FRAMES stays the absolute target above.
...
# Default: always warm-start from the snapshot (a fixed 10M run). RESUME=1
# with an existing train_state.pt instead continues that state, for when a
# previous invocation was killed outright and its partial frames are wanted.
SUPERVISOR_ARGS=()
...
mkdir -p logs
LOG="logs/train-deck-pinned-150m-$(date +%Y%m%dT%H%M%S).log"
