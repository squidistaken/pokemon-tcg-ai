...
def _structured_env(seed: int = 0) -> EnvBase:
    ...
    """
    env = TCGEnv(
        DECK, DECK, seed=seed, encoder=StructuredObservationEncoder(max_options=MAX_OPTIONS)
    )
...
def test_operator_writes_int64_action(structured_model_cfg, structured_obs_spec, action_spec) -> None:
    """
    The policy operator matches the RandomMaskedPolicy contract: it reads the
    mask and writes an int64 ``action``.
    """
    operator = build_ppo_actor_critic(structured_model_cfg, structured_obs_spec, action_spec)
...
def test_actions_never_illegal(structured_model_cfg, structured_obs_spec, action_spec) -> None:
    """
...
def test_rollout_respects_mask(structured_model_cfg, structured_obs_spec, action_spec) -> None:
    """
